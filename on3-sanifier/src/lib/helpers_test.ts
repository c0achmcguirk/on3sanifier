import { On3Helpers, getReactionCount, colorCodePostsByReactions, filterPosts, filterThreads } from './helpers';
...
describe('getReactionCount', () => {
  ...
  it('should return the count of bdi tags', () => {
    const post = document.createElement('div');
    post.innerHTML = `<a class="reactionsBar-link"><bdi>User1</bdi>, <bdi>User2</bdi></a>`;
  ...
  it('should return the sum of bdi tags and "and X others"', () => {
    const post = document.createElement('div');
    post.innerHTML = `<a class="reactionsBar-link"><bdi>User1</bdi>, <bdi>User2</bdi> and 3 others</a>`;
    expect(getReactionCount(post)).toBe(5);
  });
...
describe('filterPosts', () => {
  ...
  it('should not hide a post by a user in alwaysShowUsers', () => {
    ...
    const settings = { blockedUsers: ['alwaysshowuser'], alwaysShowUsers: ['alwaysshowuser'] };
    filterPosts(settings, document);
...
describe('filterThreads', () => {
  ...
  it('should hide a thread by its ID', () => {
    ...
    thread.innerHTML = `<div class="structItem-title"><a href="#">Thread Title</a></div>`;
  ...
  it('should hide a thread by a keyword in its title', () => {
    const thread = document.createElement('div');
    thread.className = 'structItem--thread';
    thread.innerHTML = `<div class="structItem-title"><a href="#">a thread with a keyword</a></div>`;
    document.body.appendChild(thread);
